...
function inferCommand(pitfall) {
  const attempted = String(pitfall.attempted ?? "").trim();
  if (looksLikeCommand(attempted)) {
    return attempted;
  }
 ...
 */
export function suggestGateFromPitfall(pitfall) {
  const failureType = String(pitfall?.failureType ?? "");
  return {
    name: inferGateName(pitfall),
    command: inferCommand(pitfall),
 ...
 * @returns {{ name: string, command: string, required: boolean, description: string, triggeredBy: string }[]}
 */
export function suggestGatesFromPitfalls(pitfalls) {
  ...
  return pitfalls
    ...
    .map((pitfall) => suggestGateFromPitfall(pitfall));
}
