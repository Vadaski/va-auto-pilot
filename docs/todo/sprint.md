...
| AP-104 | Runtime proof for observability events and evidence bundles | 2026-07-01 | Runtime loop now emits task.started/task.command/task.gate/task.completed/task.failed events, materializes per-task evidence bundles with redacted manifests, wires orchestrate worker runId into evidence, and excludes runtime evidence from partial-dispatch landed-code detection. Regression proof: orchestrate-cli full governed cycle validates checkpoint evidence paths, global event log entries, bundle manifest/events/redacted artifacts, and tracked manifest; cycle-restart validates generated evidence bundles and tracked event log. Gates PASS: check:observability, targeted orchestrate-cli/cycle-restart flows, typecheck, check:units, check:all, check:e2e, git diff --check. |
| AP-105 | Runtime proof for governance checkpoints and permission scope enforcement | 2026-07-01 | Runtime governance and permission proof landed. Plan-review gating remains enforced; orchestrate CLI flow now proves stale checkpoint dispatch blocks for sprint-state, human-board, and git HEAD drift with checkpoint.stale event-log evidence. Worker dispatch now emits task.command permission decisions, blocks destructive worker commands before spawn, and fails out-of-scope worker diffs before Review. Governed flow captures worker prompt permission scope, asserts out-of-scope/destructive enforcement, journal signals, and event-log permission decisions. Gates PASS: typecheck, check:units, targeted orchestrate-cli flow, check:all, check:e2e, git diff --check. |
| AP-106 | Runtime proof for budget guardrails and eval gate history | 2026-07-01 | Runtime proof added in test-flows/auto-pilot-loop-cli.yaml for budget soft warn/hard stop journal evidence and eval PASS/FAIL/AMBIGUOUS task transitions. Verified .va-auto-pilot/evidence/eval-history.jsonl records, eval-compare --json pass rate, ambiguous required eval blocks Done. Validation: node scripts/test-cli-flows.mjs --flow test-flows/auto-pilot-loop-cli.yaml; npm run typecheck; npm run check:units; npm run check:all; npm run check:e2e; git diff --check. |
